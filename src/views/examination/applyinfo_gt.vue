<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <div style="margin-top:10px;">
        ...
        <div style="float: right">
          <el-button v-waves :loading="downloadLoading" type="primary" icon="el-icon-download" disabled @click="downloadzhengshu">导出
          </el-button>
          <el-checkbox v-model="showReviewer" style="margin-left:5px;" @change="tableKey=tableKey+1"/>
        </div>
      </div>
    </div>
    ...
    <el-table
      ...
      <el-table-column label="证书类型" min-width="90px">
        <template slot-scope="scope">
      ...
      <el-table-column label="送key地址" width="128px" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="left" width="200" class-name="small-padding fixed-width" fixed="right">
        <template slot-scope="scope">
          <el-button v-if="scope.row.isPass==4 && scope.row.type==0 && scope.row.payState==0" size="mini" type="danger" disabled @click="printexpress(scope.row.applyId)">打印</el-button>
          <el-button v-if="scope.row.isPass==4 && scope.row.type==1 && !scope.row.self_fetch" size="mini" type="primary" disabled @click="lingqu(scope.row)">领取</el-button>
          <el-button v-if="scope.row.isPass==4 && scope.row.type==1 && scope.row.self_fetch" size="mini" type="info" style="width: 70px" disabled>已领取</el-button>
          <el-button v-if="scope.row.isPass==4 && scope.row.payState==0" size="mini" type="primary" style="width: 90px" disabled @click="getInvoice(scope.row.applyId)">查询发票</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import { fetchList, printexpress, updateApplyInfo, changepaystatus, downloadzehngshu, getInvoices } from '@/api/examination'
...
export default {
  ...
  methods: {
    ...
    downloadzhengshu() {
      ...
      downloadzehngshu().then(response => {
        if (response.status === 200) {
          this.downloaddata = response.data
          console.log(this.downloaddata)
            import('@/vendor/Export2Excel').then(excel => {
              const tHeader = ['单位名称', '组织机构代码', '法定代表人', '注册联系地址', '有效期', '颁发单位:', '登记号', '权限', 'CN', '金额', '业务类型', '总汇']
